module CoralFunctions

export coral_data, node_model, ude_model, ude_model_from_data, phase_plane, test, state_estimates

using UniversalDiffEq, Lux, DataFrames, OrdinaryDiffEq, Distributions, Random, Plots, Revise
...
=#
function coral_data(;plot = true, seed = 123, datasize = 60, T = 300, σ1 = 0, σ2 = 0, u01 = .2, u02 = .2, a = .1, γ = .8, r = 1, d = .44, λ = constant_fun(.3))
    # set seed 
    Random.seed!(seed)
    ...
    if plot
        ...
        X = DataFrame(time = data.time)
        X.covariate = [λ(t) for t in data.time]
        return data, X, plt
    end
...
function ude_model(;speedy = false, σ1 = 0, σ2 = 0, T1 = 175, T2 = 300, u01 = .2, u02 = .2, a = .1, γ = .8, r = 1, d = .44, λ = constant_fun(.3))
    ...
    data, X, plt = coral_data(T = T2, σ1 = σ1, σ2 = σ2, u01 = u01, u02 = u02, a = a, γ = γ, r = r, d = d, λ = λ)
